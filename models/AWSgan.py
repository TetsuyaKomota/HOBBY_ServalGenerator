...
def generator_model():
    ...
    model.add(Conv2DTranspose(KERNEL_CORE_SIZE*4, (5, 5), strides=(2, 2), kernel_initializer=trunc(stddev=STDDEV), padding="same"))
    model.add(BatchNormalization(momentum=BN_M, epsilon=BN_E))
    model.add(Activation("relu"))
    model.add(Conv2DTranspose(KERNEL_CORE_SIZE*2, (5, 5), strides=(2, 2), kernel_initializer=trunc(stddev=STDDEV), padding="same"))
    model.add(BatchNormalization(momentum=BN_M, epsilon=BN_E))
    model.add(Activation("relu"))
    model.add(Conv2DTranspose(KERNEL_CORE_SIZE*1, (5, 5), strides=(2, 2), kernel_initializer=trunc(stddev=STDDEV), padding="same"))
    model.add(BatchNormalization(momentum=BN_M, epsilon=BN_E))
    model.add(Activation("relu"))
    model.add(Conv2DTranspose(                 3, (5, 5), strides=(2, 2), kernel_initializer=trunc(stddev=STDDEV), padding="same"))
    model.add(Activation("tanh"))
    return model

def discriminator_model():
    model = Sequential()
    model.add(Conv2D(KERNEL_CORE_SIZE*1, (5, 5), strides=(2, 2), kernel_initializer=rand(stddev=STDDEV), input_shape=(IMG_SIZE, IMG_SIZE, 3)))
    model.add(LeakyReLU(0.2))
    model.add(Conv2D(KERNEL_CORE_SIZE*2, (5, 5), strides=(2, 2), kernel_initializer=rand(stddev=STDDEV)))
#    model.add(BatchNormalization(momentum=BN_M, epsilon=BN_E))
    model.add(LeakyReLU(0.2))
    model.add(Conv2D(KERNEL_CORE_SIZE*4, (5, 5), strides=(2, 2), kernel_initializer=rand(stddev=STDDEV)))
#    model.add(BatchNormalization(momentum=BN_M, epsilon=BN_E))
    model.add(LeakyReLU(0.2))
    model.add(Conv2D(KERNEL_CORE_SIZE*8, (5, 5), strides=(2, 2), kernel_initializer=rand(stddev=STDDEV)))
#    model.add(BatchNormalization(momentum=BN_M, epsilon=BN_E))
    model.add(LeakyReLU(0.2))
    model.add(Flatten())
    # model.add(Dropout(0.5))
